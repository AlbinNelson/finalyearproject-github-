...
export default function Home() {
  return (
    <>
      <section className="h-full w-full pt-48 relative flex items-center justify-center flex-col">
      <div className="absolute bottom-0 left-0 right-0 top-0 bg-[linear-gradient(to_right,#161616_1px,transparent_1px),linear-gradient(to_bottom,#161616_1px,transparent_1px)] bg-[size:4rem_4rem] [mask-image:radial-gradient(ellipse_60%_50%_at_50%_0%,#000_70%,transparent_110%)]"></div>
        <div className="bg-gradient-to-r from-primary to-secondary text-transparent bg-clip-text relative">
          <h1 className="text-9xl font-bold text-center md:text-[250px]">WebBuild</h1>
        </div>
        <div className="flex justify-center items-center realtive md:mt-[-70px] z-10">
         <Image src={'/assets/Screenshot 2024-10-24 232938.png'}
                alt="banner image"
                height={1200}
                width={1200}
      ...
      <section className="flex justify-center items-center flex-col gap-4 mt-20 md:mt-32 lg:mt-48 xl:mt-48">
        <h2 className="text-4xl text-center">Choose what fits your right</h2>
        <p className="text-muted-foreground text-center">Our straight forward pricing plans are tailored to meet your needs. If {"you're"} not <br/>
          ...
          {pricingCards.map((card)=>(
            ...
            <Card key={card.title} className={clsx('w-[300px] flex flex-col justify-between',{"border-2 border-primary":card.title==='Unlimited Saas'})}>
              ...
              <CardFooter className="flex flex-col items-start gap-4">
                <div>{card.features.map((features)=>
