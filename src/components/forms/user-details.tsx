...
const UserDetails=({id,type,userData,subAccounts}: Props) => {
    ...
    const userDataSchema = z.object({
      ...
      const onChangePermission = async (
        ...
        // Optimistically update permissions state
        if (subAccountPermissions) {
          const updatedPermissions = subAccountPermissions.Permissions.map((perm) =>
            perm.subAccountId === subAccountId ? { ...perm, access: val } : perm
          );
          setSubAccountsPermission({ ...subAccountPermissions, Permissions: updatedPermissions });
        }
        ...
        try {
          ...
          // Save activity logs if the type is 'agency'
          if (type === "agency") {
            await saveActivityLogsNotification({
          ...
          if (subAccountPermissions) {
            const revertedPermissions = subAccountPermissions.Permissions.map((perm) =>
              perm.subAccountId === subAccountId ? { ...perm, access: !val } : perm
            );
            setSubAccountsPermission({ ...subAccountPermissions, Permissions: revertedPermissions });
          }
          ...
          // Stop loading and refresh the page
    ...
    return(
        <Card className="w-full">
            <CardHeader>
                ...
                <CardDescription>Add or update your info</CardDescription>
            </CardHeader>
            <CardContent>
                <Form {...form}>
                        ...
                        <FormField 
                            ...
                            render={({field}) => <FormItem className="flex-1">
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        <SelectItem value="AGENCY_ADMN">Agency Admin</SelectItem>
